...
$query = mysqli_query($dbc, "SELECT id FROM workshop ORDER BY id DESC LIMIT 0,1");
...
&& in_array($extension, $allowedExts))
{
	...
	else
	{
		...
		if (file_exists("uploads/w_events/" . $_FILES["file"]["name"]))
		{
		unlink("uploads/w_events/" . $_FILES["file"]["name"]);
		}
		else{
			$pic=$_FILES["file"]["name"];
			$conv=explode(".",$pic);
			$ext=$conv['1'];
			move_uploaded_file($_FILES["file"]["tmp_name"],"uploads/w_events/". $user.".".$ext);
//			echo "<br>Stored in as: " . "uploads/".$user.".".$ext;
			$url=$user.".".$ext;
			
			$query2="INSERT INTO workshop(name, date, time, venue, descp, fee, cover, file) VALUES ('$name','$date','$time','$venue','$descp','$fee', 'uploads/w_events/$url', '$file')";
...
?>	  
		<form class="form-horizontal" id="addevent" action="" method="post" enctype="multipart/form-data">
			...
			<div class="col-sm-4">
				...
				<input type="text" name="name" id="event" class="form-control" placeholder="Name of event">
			</div>
			</div>
			<div class="form-group">
			<div class="col-sm-4">	
			...
			<div class="col-sm-4">	
				...
				<input type="text" name="time" id="time" class="form-control" placeholder="Time as Text">
			</div>
			</div>
			<div class="form-group">
			<div class="col-sm-4">	
			...
			<div class="col-sm-4">	
				...
				<input type="file" name="file" id="cover" class="form-control-file" placeholder="Select Cover Image">
			</div>
			</div>
			<div class="form-group">
			<div class="col-sm-4">	
				<label for="descp" class="control-label">Small Description</label>
				<input type="text" name="descp" id="descp" class="form-control" placeholder="Description">
			</div>
			</div>
			<div class="form-group">
			<div class="col-sm-4">	
