...
if(isset($_GET['m_list'])){
	$idr = $_GET['m_list'];
	$query = mysqli_query($dbc,"SELECT * FROM m_register WHERE e_id=$idr ORDER BY date DESC");
...
if(isset($_GET['deluser'])){
	if($_GET['deluser']){
		$rid = $_GET['m_list'];
		$id = $_GET['deluser'];
		$del = "UPDATE m_register SET status='Verified' WHERE t_id='$id'";
		if(mysqli_query($dbc,$del)){
	
		header('Location: m_list.php?m_list='.$rid.'&action=verified');
		exit;
		}
	}
}
...
<head>
  ...
  {
	  	window.location.href = 'm_list.php?m_list='+id+'&deluser=' + tid;
  }
...
-->
      ...
      <div class="content">
        <div class="container-fluid">
			  <div class="row">
				  <a href="m_events_add.php"><button class="btn btn-info">Add Event</button></a>
