...
$query = mysqli_query($dbc,"SELECT * FROM t_events WHERE del=1 ORDER BY id DESC");

if(isset($_GET['deluser'])){
	if($_GET['deluser']){
		$id = $_GET['deluser'];
		$del = "UPDATE t_events SET del=0 WHERE id=$id";
		if(mysqli_query($dbc,$del)){
		header('Location: t_events.php?action=deleted');
		exit;
		}
	}
}
...
<head>
  ...
  function deluser(id, title)
  {
	  ...
	  {
	  	window.location.href = 't_events.php?deluser=' + id;
	  }
  }
	function t_list(id, title)
  {
	  	window.location.href = 't_list.php?t_list=' + id;
  }
          ...
          echo"<div class='card' style='width: 400px; margin-right: 20px';>";
			  echo"<img class='card-img-top' src='$data[7]' alt='Card image cap' style='width: 100%; height:200px; margin-right: 20px'>";
				echo"<div class='card-body'>";
					 ...
					 <a href="javascript:t_list('<?php echo $data[0];?>','<?php echo $data[1];?>')" class='btn btn-info'>R-L</a>
