...
$query = mysqli_query($dbc, "SELECT id FROM t_events ORDER BY id DESC LIMIT 0,1");
...
&& in_array($extension, $allowedExts))
{
	...
	else
	{
		...
		if (file_exists("uploads/t_events/" . $_FILES["file"]["name"]))
		{
		unlink("uploads/t_events/" . $_FILES["file"]["name"]);
		}
		else{
			$pic=$_FILES["file"]["name"];
			$conv=explode(".",$pic);
			$ext=$conv['1'];
			move_uploaded_file($_FILES["file"]["tmp_name"],"uploads/t_events/". $user.".".$ext);
//			echo "<br>Stored in as: " . "uploads/".$user.".".$ext;
			$url=$user.".".$ext;
			
			$query2="INSERT INTO t_events(name, date, time, venue, descp, fee, cover, file) VALUES ('$name','$date','$time','$venue','$descp','$fee', 'uploads/t_events/$url', '$file')";
