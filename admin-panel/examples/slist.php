...
if(isset($_GET['slist'])){
	$idr = $_GET['slist'];
	$query = mysqli_query($dbc,"SELECT * FROM siphon_register WHERE e_id=$idr ORDER BY date DESC");
...
if(isset($_GET['deluser'])){
	if($_GET['deluser']){
		$rid = $_GET['slist'];
		$id = $_GET['deluser'];
		$del = "UPDATE siphon_register SET status='Verified' WHERE t_id='$id'";
		if(mysqli_query($dbc,$del)){
	
		header('Location: slist.php?slist='.$rid.'&action=verified');
		exit;
		}
	}
}
...
<head>
  ...
  {
	  	window.location.href = 'slist.php?slist='+id+'&deluser=' + tid;
  }
...
-->
      ...
      <div class="content">
        <div class="container-fluid">
			  <div class="row">
				  <a href="addevent.php"><button class="btn btn-info">Add Event</button></a>
                    ...
                    <table id="headerTable" class="table">
                      ...
                      <tbody>
							...
							while($data = mysqli_fetch_row($query)){
								...
								<a class="btn btn-success" href="javascript:deluser('<?php echo $data[1];?>','<?php echo $data[4];?>')">Verified</a>
								<?php
									}
								echo "</td>";
								echo "</tr>";
