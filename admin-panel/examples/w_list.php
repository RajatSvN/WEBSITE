...
if(isset($_GET['w_list'])){
	$idr = $_GET['w_list'];
	$query = mysqli_query($dbc,"SELECT * FROM w_register WHERE e_id=$idr ORDER BY date DESC");
...
if(isset($_GET['deluser'])){
	if($_GET['deluser']){
		$rid = $_GET['w_list'];
		$id = $_GET['deluser'];
		$del = "UPDATE w_register SET status='Verified' WHERE t_id='$id'";
		if(mysqli_query($dbc,$del)){
	
		header('Location: w_list.php?w_list='.$rid.'&action=verified');
		exit;
		}
	}
}
...
<head>
  ...
  {
	  	window.location.href = 'w_list.php?w_list='+id+'&deluser=' + tid;
  }
