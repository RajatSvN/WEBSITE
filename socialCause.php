<<<<<<< HEAD:socialCause.php
<!DOCTYPE html>
<html>
<head>
	<title>Social Causes</title>
	<meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
	<link rel="stylesheet" type="text/css" href="../css/socialCause.css">
	<link rel="stylesheet" href="../css/floatdesktopnavbar.css">
</head>
<body>
		<!---------------DESKTOP NAVBAR------------------------->
		<div class="navbarparent">
				<div class="navba" id="botnav">
				<ul>
				 
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/calendar.png" class="icon"><br>Schedule</a>
				  </li>
				 
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/workshop3.png" class="icon"><br>Workshop</a>
				  </li>
				
				  <li class="nav-item"> 
					
					<a class="nav-link" href="#"><img src="../images/contact.png" class="icon"><br>Contact</a>
					
				  </li>
				  
				  <li class="nav-item"> 
					
					<a class="nav-link" href="#"><img src="../images/logo.png" class="mainicon"></a>
				  </li>
		
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/FeedBack.png" class="icon"><br>Feedback</a>
				  </li>
		
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/party.png" class="icon"><br>Events</a>
				  </li> 
				   
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/sponsors.png" class="icon"><br>Sponsors</a>    
				  </li>
		
				</ul>
				</div>
			</div>
	<!----------------------------------------------------->
	<div style="width:100%;background-image: linear-gradient(to right,#1c92d2, transparent 80%,#f2fcfe, transparent 50%),url(../images/Beautifulback2.png);">
		<div class="container">

			<div class="row featurette">
				<div class="col-md-5">
					<img src="../images/sample.jpg" width="100%">
				</div>
				<div class="col-md-7">
					<h2 class="featurette-heading">Heading: It will kinda look long just because, why not?</h2>
						<p class="lead">
							Just a sample text to show you how it would actually look like on the screen when loaded...
						</p>
					</div>
				</div>				
			</div>
		</div>
		<div style="width:100%;background-image: linear-gradient(to left,#800080, transparent 80%,#ffc0cb, transparent 50%),url(../images/Beautyback.png);">
			<div class="container">
			<div class="row featurette">
				<div class="col-md-7">
					<h2 class="featurette-heading">Heading: It will kinda look long just because, why not?</h2>
						<p class="lead">
							Just a sample text to show you how it would actually look like on the screen when loaded...
						</p>
				</div>
				<div class="col-md-5">
					<img src="../images/sample.jpg" width="100%">
				</div>
				</div>				
			</div>
		</div>
		<div style="width:100%;background-image: linear-gradient(to right,#007551, transparent 80%,#78ffde, transparent 50%),url(../images/TimeTravel.png);">
			<div class="container">
			<div class="row featurette">
				<div class="col-md-5">
					<img src="../images/sample.jpg" width="100%">
				</div>
				<div class="col-md-7">
					<h2 class="featurette-heading">Heading: It will kinda look long just because, why not?</h2>
						<p class="lead">
							Just a sample text to show you how it would actually look like on the screen when loaded...
						</p>
					</div>
				</div>				
			</div>
			<div class="container-fluid">
			<br>
			<br>
			<br>
			<br>
			<br>
		</div>
	</div>
</body>
=======
<!DOCTYPE html>
<html>
<head>
	<title>Social Causes</title>
	<meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
	<link rel="stylesheet" type="text/css" href="../css/socialCause.css">
	<link rel="stylesheet" href="../css/floatdesktopnavbar.css">
</head>
<body>
		<!---------------DESKTOP NAVBAR------------------------->
		<div class="navbarparent">
				<div class="navba" id="botnav">
				<ul>
				 
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/calendar.png" class="icon"><br>Schedule</a>
				  </li>
				 
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/workshop3.png" class="icon"><br>Workshop</a>
				  </li>
				
				  <li class="nav-item"> 
					
					<a class="nav-link" href="#"><img src="../images/contact.png" class="icon"><br>Contact</a>
					
				  </li>
				  
				  <li class="nav-item"> 
					
					<a class="nav-link" href="#"><img src="../images/logo.png" class="mainicon"></a>
				  </li>
		
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/FeedBack.png" class="icon"><br>Feedback</a>
				  </li>
		
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/party.png" class="icon"><br>Events</a>
				  </li> 
				   
				  <li class="nav-item">
					
					<a class="nav-link" href="#"><img src="../images/sponsors.png" class="icon"><br>Sponsors</a>    
				  </li>
		
				</ul>
				</div>
			</div>
	<!----------------------------------------------------->
	<div style="width:100%;background-image: linear-gradient(to right,#1c92d2, transparent 80%,#f2fcfe, transparent 50%),url(../images/Beautifulback2.png);">
		<div class="container">

			<div class="row featurette">
				<div class="col-md-5">
					<img src="../images/sample.jpg" width="100%">
				</div>
				<div class="col-md-7">
					<h2 class="featurette-heading">Heading: It will kinda look long just because, why not?</h2>
						<p class="lead">
							Just a sample text to show you how it would actually look like on the screen when loaded...
						</p>
					</div>
				</div>				
			</div>
		</div>
		<div style="width:100%;background-image: linear-gradient(to left,#800080, transparent 80%,#ffc0cb, transparent 50%),url(../images/Beautyback.png);">
			<div class="container">
			<div class="row featurette">
				<div class="col-md-7">
					<h2 class="featurette-heading">Heading: It will kinda look long just because, why not?</h2>
						<p class="lead">
							Just a sample text to show you how it would actually look like on the screen when loaded...
						</p>
				</div>
				<div class="col-md-5">
					<img src="../images/sample.jpg" width="100%">
				</div>
				</div>				
			</div>
		</div>
		<div style="width:100%;background-image: linear-gradient(to right,#007551, transparent 80%,#78ffde, transparent 50%),url(../images/TimeTravel.png);">
			<div class="container">
			<div class="row featurette">
				<div class="col-md-5">
					<img src="../images/sample.jpg" width="100%">
				</div>
				<div class="col-md-7">
					<h2 class="featurette-heading">Heading: It will kinda look long just because, why not?</h2>
						<p class="lead">
							Just a sample text to show you how it would actually look like on the screen when loaded...
						</p>
					</div>
				</div>				
			</div>
			<div class="container-fluid">
			<br>
			<br>
			<br>
			<br>
			<br>
		</div>
	</div>
</body>
>>>>>>> 191a99112acb0681580b383869d28905a00e3bf3:HTML/socialCause.html
</html>